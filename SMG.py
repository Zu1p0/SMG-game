...
Escape the Maze (autogenerated Maze)
Exit is blue cell, that will send you to a larger Maze
Entrance is green cell.

controls:
    space: jump
    q: quit
    r: restart from entrance
    w: forward
...
def start_game():
    ...
    genmaze_entity = Text(text="Generating Maze...", position=window.center,color=color.black)
    ...
    resume()
    destroy(genmaze_entity,delay=1)
...
def start_screen():
    ...
    instructions=Text(text="Escape the Maze (autogenerated Maze)\nExit is blue cell, that will send you to a larger Maze\nEntrance is green cell.\n\ncontrols\n     space: jump; q: quit; r: restart from entrance; w: forward", position=(-0.5,-0.3,0),color=color.white)
    ...
    class KeyClick(Button):
        ...
        def update(self):
            if held_keys["enter"] :
                ...
                data.label.changeText("Maze size: "+str(data.level))
...
class LevelLabel(Text):
    def __init__(self, **kwargs):
        ...
        self.position=window.top_right
        self.x = -0.1
        self.y = -0.1

    def changeText(self,Mylabel):
        global data
        self.text=Mylabel
        self.position=window.top_right
        self.x = self.x - 0.01 - self.width
        self.y = self.y - 0.03
    ...
    if data.player != None:
        ...
        if round(data.player.x) == data.player.endpoint[0]: #and round(data.player.z) == data.player.endpoint[2]:
            ...
            if data.level > 30 :
                ...
                data.opti = True
            start_game() 
        if round(data.player.y) <-10:
            data.label.changeText("Failing... (q to quit)")
    if held_keys["q"]: quit()
